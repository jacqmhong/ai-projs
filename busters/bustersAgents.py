...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        mostLikelyPositions = []
        for posBeliefDist in livingGhostPositionDistributions:
            mostLikelyPositions.append(posBeliefDist.argMax())

        mazeDistances = []
        for ghostPosition in mostLikelyPositions:
            mazeDistances.append(self.distancer.getDistance(pacmanPosition, ghostPosition))

        closestGhost = mostLikelyPositions[mazeDistances.index(min(mazeDistances))]

        successorDistances = []
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            successorDistances.append(self.distancer.getDistance(successorPosition, closestGhost))
